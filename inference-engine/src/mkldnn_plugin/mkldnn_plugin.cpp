...
InferenceEngine::IExecutableNetworkInternal::Ptr Engine::ImportNetwork(
    ...
    std::fstream inputStream(modelFileName, std::ios::in | std::ios::binary);
    if (inputStream.fail()) {
        IE_THROW() << "Cannot open file to import model: " << modelFileName;
    }
    return ImportNetwork(inputStream, config);
}
...
InferenceEngine::IExecutableNetworkInternal::Ptr Engine::ImportNetwork(std::istream& networkModel,
    ...
    CNNNetwork cnnnetwork = GetCore()->ReadNetwork(xmlString, std::move(dataBlob), extensionManager->Extensions());
