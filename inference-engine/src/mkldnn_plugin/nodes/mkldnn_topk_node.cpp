...
void MKLDNNTopKNode::createPrimitive() {
    ...
    if (jit_mode) {
        ...
        auto ceil_pow_2 = [&](size_t n, int &p, int &cnt, bool cmp_val) {
            int m = n - 1;
            int log_p = 0;
            p = 1;
            while (m) {
            ...
            int *buf;
            if (cmp_val) {
                bitonic_idx_buf.reset(new int[max_cnt]);
                buf = bitonic_idx_buf.get();
            } else {
                bitonic_k_idx_buf.reset(new int[max_cnt]);
                buf = bitonic_k_idx_buf.get();
            }

            bitonic_push_idx(p, n, buf, cnt, cmp_val);
        };
        ...
        if (algorithm == TopKAlgorithm::topk_bitonic_sort) {
            ceil_pow_2(axis_dim, jcp.bitonic_size, jcp.bitonic_idx_cnt, true);
            if (sort_index) {
                ceil_pow_2(top_k, jcp.bitonic_k_size, jcp.bitonic_k_idx_cnt, false);
            }
        }
...
inline void MKLDNNTopKNode::topk_kernel_process(const uint8_t *in_p, uint8_t *out_p, uint8_t *out_idx_p,
    ...
    arg.bitonic_idx_buf = bitonic_idx_buf.get();
    arg.bitonic_k_idx_buf = bitonic_k_idx_buf.get();
    (*topk_kernel)(&arg);
}
...
inline void MKLDNNTopKNode::bitonic_push_idx(int p, int n, int *buf, int &cnt, bool cmp_val) {
    ...
    for (int len = 2; len < p; len <<= 1) {
        for (int start = 0; start < p; start += len) {
            ...
            for (int i = sub_len - 1; start + len - i - 1 < n && i >= 0; i--) {
                buf[cnt++] = (start + i) * sort_stride;
                buf[cnt++] = (start + len - i - 1) * sort_stride;
            }
            for (; sub_len > 0; sub_len >>= 1) {
                for (int sub_start = start; sub_start < start + len; sub_start += sub_len) {
                    ...
                    for (int j = 0; sub_start + j + minor_len < n && j < minor_len; j++) {
                        buf[cnt++] = (sub_start + j) * sort_stride;
                        buf[cnt++] = (sub_start + j + minor_len) * sort_stride;
                    }
                }
            }
        }
    ...
    for (int i = sub_p - 1; p - i - 1 < n && i >= 0; i--) {
        buf[cnt++] = i * sort_stride;
        buf[cnt++] = (p - i - 1) * sort_stride;
    }
    for (; sub_p > 0; sub_p >>= 1) {
        ...
        for (int sub_start = 0; (!cmp_val || (cmp_val && sub_start < n)) && sub_start < p;
            ...
            for (int j = 0; sub_start + j + minor_p < n && j < minor_p; j++) {
                buf[cnt++] = (sub_start + j) * sort_stride;
                buf[cnt++] = (sub_start + j + minor_p) * sort_stride;
            }
        }
    }
}
