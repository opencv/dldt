...
std::shared_ptr<ngraph::Function> get_initial_function(const bool& fq,
    ...
    auto result = createFunction(fq, model, input_params, filters_shape, conv_stride, conv_dilation, bias_shape,
        maxpool_stride, maxpool_shape, &graph_data , &conv_data);
...
void Decompose2DConvTestInvalidFixture::SetUp() {
    ...
    function = get_initial_function(fq, model, input_shape, filters_shape, conv_stride, conv_dilation,
        bias_shape, maxpool_stride, maxpool_shape, graph_data, conv_data);
...
void Decompose2DConvTestFixture::SetUp() {
    ...
    function = get_initial_function(fq, model, input_shape, filters_shape, conv_stride, conv_dilation,
        bias_shape, maxpool_stride, maxpool_shape, graph_data, conv_data);
    reference_function = get_reference(fq, model, input_shape, graph_data, conv_data);
}

std::shared_ptr<ngraph::Node> CreateBiasConst(std::shared_ptr<ngraph::opset7::Add> conv_bias, const ConvData& conv_data) {
...
ngraph::OutputVector SplitInput(const GraphData& graph_data, ConvData& conv_data) {
...
std::vector<std::shared_ptr<ngraph::opset7::Constant>> SplitFilters(const GraphData& graph_data, ConvData& conv_data) {
...
void TransformInput(const GraphData& graph_data, const ConvData& conv_data, ngraph::Output<ngraph::Node>& split_input_plane) {
    /*
...
std::shared_ptr<ngraph::Node> Create1DConv(const GraphData& graph_data, const ConvData& conv_data, const ngraph::Output<ngraph::Node>& input,
...
std::shared_ptr<ngraph::Node> CreateDeomposedConv(const GraphData& graph_data, ConvData& conv_data,
...
std::shared_ptr<ngraph::opset7::Result> Decompose(const GraphData& graph_data, ConvData& conv_data) {
...
std::shared_ptr<ngraph::Function> Decompose2DConvTestFixture::get_reference(const bool& fq,
    ...
    ShouldDecompose(graph_data, conv_data);

    if (model != modelType::TranspConvTransp) {
        graph_data.bias_const = CreateBiasConst(std::dynamic_pointer_cast<ngraph::opset7::Add>(graph_data.bias), conv_data);
    }

    // Create decomposed reference function
    std::shared_ptr<ngraph::opset7::Result> result;
    result = Decompose(graph_data, conv_data);
    return std::make_shared<ngraph::Function>(ngraph::ResultVector{result}, ngraph::ParameterVector{input_params});
}
